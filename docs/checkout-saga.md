...
The saga's `Schedule(...)` timeout needs a message scheduler. The standard Aspire `rabbitmq` image does **not** ship the `rabbitmq_delayed_message_exchange` plugin, so `UseDelayedMessageScheduler()` is not an option here, and `UseInMemoryScheduler()` only exists for the in-memory transport (not RabbitMQ). v8 therefore uses the **in-memory Quartz scheduler** (`MassTransit.Quartz`): `AddQuartz()` + `AddQuartzHostedService()` plus `x.AddQuartzConsumers()` / `x.AddPublishMessageScheduler()` and `cfg.UsePublishMessageScheduler()` on the bus. It needs no broker plugin and is self-contained to Checkout.API. **Trade-off:** the in-memory Quartz store does not persist jobs, so a scheduled timeout does NOT survive a Checkout.API restart. For production, enable the RabbitMQ delayed-message plugin (e.g. the `masstransit/rabbitmq` image) and switch to `UseDelayedMessageScheduler()`, or back Quartz with a persistent store.
