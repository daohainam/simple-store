...
public class IdentityApiClient : IIdentityApiClient
{
    ...
    {
        using var response = await _http.PostAsJsonAsync("api/identity/login", request, cancellationToken);
    ...
    {
        using var response = await _http.PostAsJsonAsync("api/identity/register", request, cancellationToken);
    ...
    {
        using var response = await _http.PostAsJsonAsync("api/identity/refresh", request, cancellationToken);
        if (response.StatusCode is HttpStatusCode.Unauthorized or HttpStatusCode.BadRequest) return null;
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<LoginResponse>(cancellationToken);
    }
    ...
    {
        using var response = await _http.PostAsJsonAsync("api/identity/logout", request, cancellationToken);
    ...
    {
        using var response = await _http.PostAsync("api/identity/passkey/assertion-options", content: null, cancellationToken);
        response.EnsureSuccessStatusCode();
    ...
    {
        using var response = await _http.PostAsJsonAsync("api/identity/passkey/assertion", request, cancellationToken);
        if (response.StatusCode is HttpStatusCode.Unauthorized or HttpStatusCode.BadRequest) return null;
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadFromJsonAsync<LoginResponse>(cancellationToken);
    }
    ...
    {
        using var response = await _http.GetAsync("api/identity/me", cancellationToken);
    ...
    {
        using var response = await _http.PutAsJsonAsync("api/identity/me", request, cancellationToken);
        response.EnsureSuccessStatusCode();
    }

    public async Task<string> GetPasskeyCreationOptionsAsync(CancellationToken cancellationToken = default)
    {
        using var response = await _http.PostAsync("api/identity/passkey/creation-options", content: null, cancellationToken);
        response.EnsureSuccessStatusCode();
        return await response.Content.ReadAsStringAsync(cancellationToken);
    }
    ...
    {
        using var response = await _http.PostAsJsonAsync("api/identity/passkey/attestation", request, cancellationToken);
        response.EnsureSuccessStatusCode();
    }

    public async Task<IReadOnlyList<UserPasskeyInfo>> GetPasskeysAsync(CancellationToken cancellationToken = default)
    {
        var list = await _http.GetFromJsonAsync<List<UserPasskeyInfo>>("api/identity/passkeys", cancellationToken);
    ...
    {
        using var response = await _http.DeleteAsync($"api/identity/passkeys/{Uri.EscapeDataString(credentialIdBase64)}", cancellationToken);
        response.EnsureSuccessStatusCode();
    }
    ...
    {
        var query = $"api/identity/users?page={page}&pageSize={pageSize}";
    ...
    public async Task<int> GetUserCountAsync(CancellationToken cancellationToken = default) =>
        await _http.GetFromJsonAsync<int>("api/identity/users/count", cancellationToken);

    public async Task<UserInfo?> GetUserByIdAsync(string id, CancellationToken cancellationToken = default)
    {
        using var response = await _http.GetAsync($"api/identity/users/{Uri.EscapeDataString(id)}", cancellationToken);
    ...
    {
        using var response = await _http.PutAsJsonAsync($"api/identity/users/{Uri.EscapeDataString(id)}", request, cancellationToken);
        response.EnsureSuccessStatusCode();
    }

    public async Task LockUserAsync(string id, CancellationToken cancellationToken = default)
    {
        using var response = await _http.PostAsync($"api/identity/users/{Uri.EscapeDataString(id)}/lock", content: null, cancellationToken);
        response.EnsureSuccessStatusCode();
    }

    public async Task UnlockUserAsync(string id, CancellationToken cancellationToken = default)
    {
        using var response = await _http.PostAsync($"api/identity/users/{Uri.EscapeDataString(id)}/unlock", content: null, cancellationToken);
        response.EnsureSuccessStatusCode();
    }
}
