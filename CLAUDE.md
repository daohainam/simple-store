...
Run everything (Aspire orchestrates Postgres + PgWeb + Redis + RedisInsight + RabbitMQ + Management UI + KurrentDB + Identity API + Catalog API + Order API + Cart API + Inventory API + Gateway + Web + Admin):
...
Aspire AppHost ([src/SimpleStore.AppHost/AppHost.cs](src/SimpleStore.AppHost/AppHost.cs)) is the entry point. It provisions one `postgres` resource with PgWeb and four logical databases (`catalogdb`, `orderdb`, `identitydb`, `inventorydb`), one `cart-redis` resource with RedisInsight, one `rabbitmq` resource with the Management plugin, and one `kurrentdb` resource (with a `kurrentdb-data` volume) for the Inventory write side; each microservice is the only resource that touches its own data store, `order` / `catalog` / `cart` reference `rabbitmq`, and `inventory` references `kurrentdb`. The AppHost also defines three parameters — `jwt-key` (secret), `jwt-issuer`, `jwt-audience` — and propagates them as `Jwt__Key` / `Jwt__Issuer` / `Jwt__Audience` env vars to every service that issues or validates JWTs (including the `gateway`). Web and Admin only reference the `gateway` (YARP reverse proxy) — they no longer reference backend services directly. The `gateway` itself references every backend service (`identity`, `catalog`, `order`, `cart`, `inventory`).
